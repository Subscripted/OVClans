package dev.subscripted.eloriseClans.database.connections;
...
public class Coins {
    ...
    public void setPlayerData(UUID uuid, int defaultCoins, int defaultBankCoins) {
        ...
        try (PreparedStatement checkStmt = mySQL.getConnection().prepareStatement(checkQuery)) {
            ...
            if (count == 0) {
                try (PreparedStatement insertStmt = mySQL.getConnection().prepareStatement(insertQuery)) {
